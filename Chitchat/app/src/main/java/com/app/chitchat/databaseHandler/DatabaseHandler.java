...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public long insertChat(Chat chat){
        ...
        chatData.put(CHAT_COLUMNS[6], chat.get_id());
    ...
    public String getUserBy(String userId){
    ...
    public long insertMessage(String fromUserId, com.app.chitchat.data.firebaseData.Message msg) throws SQLException {
        if(!isExistingUser(fromUserId)){
            return -1;
        }
        ...
        throw new SQLException("Unable to insert msg!");
    }

    public int updateLastMsgId(String chatId, int pos){
    ...
    public Profile readUserFrom(Cursor userCursor){
        if(userCursor.getColumnCount() == 4){
            return new Profile(userCursor.getString(0), userCursor.getString(1), userCursor.getString(2), userCursor.getInt(3)==0);
        }
        return new Profile(userCursor.getString(0), userCursor.getString(1), userCursor.getString(2), userCursor.getString(3), userCursor.getInt(4)==0);
    }

    public void createMessageTable(String userID){
        String q = "CREATE TABLE "+ getMessageTableName(userID) +" (" +
                MSG_COLUMNS[0]+ " INTEGER AUTOINCREMENT PRIMARY KEY," +
    ...
    public Message getMessageById(String userId, int msgId){
    ...
    private Message readMsgFrom(Cursor msgCursor) {
        return new Message(msgCursor.getInt(0), msgCursor.getString(1), msgCursor.getString(2), msgCursor.getInt(3));
    }
    ...
    public void createSimpleMsgBodyTable(String userId){
        String q = "CREATE TABLE "+getMsgBodyTableName(userId)+" (" +
                SIMPLE_MSG_BODY_COLUMNS[0] +" INTEGER AUTOINCREMENT PRIMARY KEY," +
    ...
    public static String getMsgBodyTableName(String userId){
        return userId+"_msg_body";
    }

    public SimpleMessageBody getSimpleMsgBy(String userId, int msgID){
        Cursor cursor = getReadableDatabase().query(getMsgBodyTableName(userId), SIMPLE_MSG_BODY_COLUMNS, "_id=?", new String[]{String.valueOf(msgID)}, null, null, null);
    ...
    public String getSimpleMsgContent(String userId, int msgId){
        Cursor cursor = getReadableDatabase().query(getMessageTableName(userId), new String[]{SIMPLE_MSG_BODY_COLUMNS[2]}, "_id = ?", new String[]{String.valueOf(msgId)}, null, null, null);
        String content = null;
        if(cursor!=null && cursor.moveToFirst()){
    ...
    private SimpleMessageBody readSimpleMsgFrom(Cursor cursor) {
        return new SimpleMessageBody(cursor.getInt(1), cursor.getInt(1), cursor.getString(2));
    }
